...
// network variables
const byte mac[] = {0x02,0x01,0x02,0x03,0x04,0x05};
IPAddress ip(192,168,0,202); // ip of device
const unsigned int localPort = 4032; // Chones port to comunicate over
...
void turnRight()
{
  ...
  for (byte i=0; i<3; i++)
  {
    ...
    drive[0] = 1; drive[1] = 0; drive[2] = 0; drive[3] = 1;
...
void turnLeft()
{
  ...
  for (byte i=0; i<3; i++)
  {
    ...
    drive[0] = 0; drive[1] = 1; drive[2] = 1; drive[3] = 0;
    motoDrive(drive); // turn robot
    delay(10);
...
void moveForwards()
{
  ...
  for (byte i=0; i<4; i++)
  {
    // move forwards
    // B F B F
    drive[0] = 0; drive[1] = 1; drive[2] = 0; drive[3] = 1;
    motoDrive(drive); // turn robot
    delay(10);
